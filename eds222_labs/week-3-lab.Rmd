...
```{r}
# problem 1
summary(lm(size ~ latitude, pie_crab)) # Summary of the data frame?
...
#size_reg <- lm(pie_crab$size ~ pie_crab$latitude)
#time_reg <- lm(pie_crab$water_temp ~  )

#resid will give you allthe errors

```

```{r}
test <- resid(lm(pie_crab$size ~ pie_crab$latitude))
time_sse <- resid(lm(pie_crab$water_temp ~ pie_crab$latitude))

sum_time_sse <- time_sse %>%
  summary()

```

```{r}
lm_size <- (lm(pie_crab$size ~ pie_crab$latitude)^2)
...
mpg <- mpg
ggplot(data = mpg) +
  geom_boxplot(aes(y = hwy, x = year_fct))

# horizontal line is median and median is somewhat resistant to outliers
# Half the data is belew the blue horizontal line
...
2.  What does the intercept tell you, in words?
    ...
    The intercept is 42.46

    The intercept happens when vintage is 0

    Categorical variable

    hwy + bete0 + beta1 x vintage + uncertainty

    regression extends variable? In this example we are adding one coefficient

    hwy \| year_fct \| vinatge \| vintage = 2012
    ...
    Intercept is beta 0, so I guess vintage is the 0 or 1s?

    Then sometimes beta 1 is slope?

    beta 0 = the mean in 1999

    beta 1 is the difference ? telling us the difference is tiny?

3.  

    What does the coefficient estimate on the 2008 vintage indicator variable tell you, in words? **Answers:**

```{r}
car_sum <- summary(lm(mpg$hwy ~ mpg$year_fct))
