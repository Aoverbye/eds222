...
1.  What do you think each row represents?
    ...
    **Demographics and water safety in a given district in a given year**
    ...
    Each row represents, there are some with the same id that are different fiscal years There are zero or ones bc whether or not there is an investigation in that state it would be binary

2.  What columns do you think represent:

    1.  Drinking water health violations? health

    2.  Percent Black and Hispanic population in the utility district?

        pctblack and pcthisp

    3.  Median household income in the utility district? medianincomehouse
...
# Other way
#drinking_water$health_violations <- as.numeric(drinking_water$health > 0) 
#drinking_water

# Mutate/ vectorized function
# This is a short cut that may need a for loop
...
```{r}
# Make plots
# This is over plotting
# This is using binning it
drinking_water_bind <- drinking_water %>%
  mutate(pcthisp = round(pcthisp * 5) / 5) %>%
  group_by(violation = mean(violation))

# We summarize with bins to make the pattern pot  
# Something didnt work here

ggplot(data = drinking_water, aes(y = violation, x = pcthisp)) +
  geom_point() +
  geom_point(data = drinking_water_bind,
             color = "darkorange")
...
log(L) = log(PDF) + log(PDF)

-log(L) = log(PDF) + log(PDF)

1.  Optimization algorithms typically find the *minimum* value. They're intended to look for valleys, not peaks. So instead of maximizing the log likelihood, we can minimize the negative log likelihood.
...
likelihood_fun <- function(coefficients, data) {
  ...
  logit_p <- coefficients["beta0"] + coefficients["beta1"] *
    data$pcthisp
...
# Plot for whole range by changing value in c 
...
4.  How would you interpret the coefficients? What do the slope and intercept mean in this context? Where is the relationship linear, and where is it non-linear?
    ...
    The probabiity of a violation increases when the hispanic population
...
```{r}
# This did everything we did before in the previous steps
pcthisp_glm <- glm(violation ~ pcthisp, 
                   family = binomial(link = "logit"), # this is the part that is different, length = logit
                   data = drinking_water)
summary(pcthisp_glm)
...
```{r}
interaction_glm <- glm(violation ~ pcthisp * medianincomehouse, 
...
predictions <- expand_grid(
  ...
  mutate(violation = predict(interaction_glm, 
                              newdata = ., # New data is . because the . is a shorthand for whatever is on the left
                              type = "response"))

# Create plot
ggplot(predictions, aes(pcthisp, medianincomehouse, fill = violation))+
  geom_raster() +
